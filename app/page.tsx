...
export default function LandingPage() {
  return (
    <div style={{ minHeight: '100vh', backgroundColor: '#05080F', color: '#EEF2FF', fontFamily: 'system-ui, sans-serif' }}>
      ...
      {/* HERO */}
      <section style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', textAlign: 'center', padding: '6rem 2rem 4rem' }}>
        <div style={{ display: 'inline-flex', alignItems: 'center', gap: '0.5rem', backgroundColor: 'rgba(0,201,167,0.1)', border: '1px solid rgba(0,201,167,0.2)', color: '#00C9A7', padding: '0.4rem 1.1rem', borderRadius: '50px', fontSize: '0.8rem', fontWeight: 600, marginBottom: '2rem' }}>
          🦷 Built for Pre-Dental & Dental Students
        </div>
        ...
        <p style={{ fontSize: '1.1rem', color: '#6B7A9A', maxWidth: '560px', lineHeight: 1.7, marginBottom: '2.5rem' }}>
          DentPath combines AI tutoring, DAT prep, application tools, clinical trackers, and career planning into one platform built specifically for the dental journey.
        </p>
        <div style={{ display: 'flex', gap: '1rem', flexWrap: 'wrap', justifyContent: 'center' }}>
          <Link href="/signup" style={{ backgroundColor: '#00C9A7', color: '#000', fontWeight: 700, fontSize: '1rem', padding: '0.9rem 2rem', borderRadius: '50px', textDecoration: 'none' }}>
            Start for Free →
          </Link>
      ...
      <section style={{ padding: '5rem 2rem', backgroundColor: '#080D18' }}>
        <div style={{ maxWidth: '1100px', margin: '0 auto' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))', gap: '1.25rem' }}>
            {[
              { icon: '📊', title: 'DAT Prep Suite', desc: 'AI-generated practice questions across all DAT subjects — biology, chemistry, orgo, PAT, math, and reading. Never the same question twice.', tag: 'Available Now' },
              { icon: '🤖', title: 'AI Tutor', desc: 'Ask anything about DAT topics or dental school subjects. Get clear explanations, mnemonics, and practice questions on demand.', tag: 'Available Now' },
              { icon: '📋', title: 'Application Tracker', desc: 'Track every school, deadline, letter of recommendation, and secondary essay in one place.', tag: 'Coming Soon' },
              { icon: '✍️', title: 'Essay Coach', desc: 'AI-powered feedback on your personal statement. Get suggestions, tone analysis, and rewrites.', tag: 'Coming Soon' },
              { icon: '🦷', title: 'Clinical Tracker', desc: 'Log every procedure, track graduation requirements, and monitor your clinical competency progress.', tag: 'Coming Soon' },
              { icon: '🏆', title: 'Career Planning', desc: 'Explore dental specialties, track residency applications, and plan your career path after graduation.', tag: 'Coming Soon' },
            ].map((f) => (
              <div key={f.title} style={{ backgroundColor: '#0D1525', border: '1px solid rgba(255,255,255,0.07)', borderRadius: '20px', padding: '1.5rem' }}>
                ...
                <span style={{ backgroundColor: f.tag === 'Available Now' ? 'rgba(0,201,167,0.1)' : 'rgba(255,255,255,0.05)', color: f.tag === 'Available Now' ? '#00C9A7' : '#6B7A9A', fontSize: '0.72rem', fontWeight: 700, padding: '0.2rem 0.7rem', borderRadius: '50px' }}>{f.tag}</span>
              </div>
            ))}
          </div>
        </div>
            ...
            {[
              {
                ...
                features: [
                  'Limited AI-generated DAT questions',
                  'AI Tutor (10 messages/day)',
                  'School database browsing',
                  'Basic experience logger',
                ],
                featured: false,
              ...
              },
              {
                ...
                features: [
                  'Unlimited AI DAT questions',
                  'Unlimited AI Tutor',
                  'Full mock exams with score reports',
                  'Application tracker',
                  'Essay coach',
                  'Interview simulator',
                  'Clinical procedure tracker',
                ],
                featured: true,
                cta: 'Start Pro'
              },
              ...
              <div key={plan.name} style={{ position: 'relative', backgroundColor: plan.featured ? 'rgba(0,201,167,0.05)' : '#0D1525', border: `1px solid ${plan.featured ? 'rgba(0,201,167,0.3)' : 'rgba(255,255,255,0.07)'}`, borderRadius: '24px', padding: '2rem' }}>
                ...
                <Link href="/signup" style={{ display: 'block', width: '100%', textAlign: 'center', padding: '0.75rem', borderRadius: '50px', fontWeight: 600, fontSize: '0.88rem', textDecoration: 'none', backgroundColor: plan.featured ? '#00C9A7' : 'transparent', color: plan.featured ? '#000' : '#EEF2FF', border: plan.featured ? 'none' : '1px solid rgba(255,255,255,0.1)', boxSizing: 'border-box' }}>
                  {plan.cta}
                </Link>
              </div>
            ))}
