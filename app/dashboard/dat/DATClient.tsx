...
// FIXED: More robust JSON extractor
function extractJSON(text: string, isArray: boolean): any {
  console.log('Raw response preview:', text.substring(0, 200))
  
  // If the response is an error message from our API
  if (text.startsWith('ERROR:') || text.startsWith('Sorry,')) {
...
  } catch (e) {
    console.log('Post-markdown parse failed')
  }

  // Find the first { or [ and last } or ]
  const startIdx = cleaned.indexOf(isArray ? '[' : '{')
...
  } catch (e) {
    console.log('Quote fixing failed')
  }

  console.error('Failed to parse JSON. Raw text:', text.substring(0, 300))
...
      setQuestion({
        question: `Error generating question: ${err.message}`,
        options: ['Try again', 'Change subject', 'Refresh page', 'Contact support'],
        correctIndex: 0,
        explanation: 'Please try again or select a different subject.',
        topic: 'Error',
        difficulty: diffLevel,
      })
    }
...
  useEffect(() => {
    if (mode === 'practice' && question && !loading && !isPrefetching) {
...
            } catch (e) {
              console.log('Pre-fetch parse failed')
            }
          }
        } catch (e) {
...
    }
  }, [mode, question, loading, activeSubject, difficulty, isPrefetching])
...
    if (question?.topic && question.topic !== 'Error') {
